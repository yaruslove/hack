...
from utils.openvino_utils import detect_ov
...
def image_input(confidence):
    ...
    ie = Core()
    model_ov = ie.read_model(model="models/yolov8s_768_weapon_int8_openvino_model/yolov8s_768_weapon.xml")
    compiled_model = ie.compile_model(model=model_ov, device_name="CPU")
    output_layer = compiled_model.output(0)

    with col2:
        detections = detect_ov(frame, compiled_model, confidence)[0]
        image_with_boxes = draw_results(detections, frame, dict_classes)
        st.image(image_with_boxes, caption='Результат модели',
                 use_column_width=True)
    return


def _display_detected_frames(conf, model, st_frame, image, is_display_tracking=None, tracker=None):
    detections = model(frame, compiled_model, conf=conf)[0]
    image_with_boxes = draw_results(detections, frame, dict_classes)
        ...
        try:
            ...
            with col2:
                ...
                while (vid_cap.isOpened()):
                    ...
                    if success:
                        _display_detected_frames(conf, compiled_model, st_frame, image)
                    else:
                        vid_cap.release()
                        break
...
def rtsp_stream(conf):
    ...
    st.sidebar.caption(
        'Пример ссылки: rtsp://rtsp:A1234567@188.170.176.190:8028/Streaming/Channels/101?transportmode=unicast&profile=Profile_1')
